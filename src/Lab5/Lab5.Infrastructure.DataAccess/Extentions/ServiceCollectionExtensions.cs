using ClassLibrary1Infrastructure.DataAccess.Repositories;
using Itmo.Dev.Platform.Postgres.Extensions;
using Itmo.Dev.Platform.Postgres.Models;
using Itmo.Dev.Platform.Postgres.Plugins;
using Itmo.ObjectOrientedProgramming.Lab5.Repositories;
using Lab5.Infrastructure.DataAccess.Plugins;
using Microsoft.Extensions.DependencyInjection;

namespace Lab5.Inftastructure.Extensions;

public static class ServiceCollectionExtensions
{
  public static IServiceCollection AddInfrastructureDataAccess(
        this IServiceCollection collection,
        Action<PostgresConnectionConfiguration> configuration)
    {
        /*collection.AddSingleton<IConnection>(new ConnectionPostgresDB(connectionString));
        collection.AddSingleton<IClientRepository, ClientRepository>();
        collection.AddSingleton<IBillRepository, BillRepository>();
        collection.AddSingleton<ILogRepository, LogPerository>();

        return collection;*/
        collection.AddPlatformPostgres(builder => builder.Configure(configuration));
        collection.AddPlatformMigrations(typeof(ServiceCollectionExtensions).Assembly);

        collection.AddSingleton<IDataSourcePlugin, MappingPlugin>();

        collection.AddScoped<IClientRepository, ClientRepository>();
        collection.AddScoped<IBillRepository, BillRepository>();

        return collection;
    }
}